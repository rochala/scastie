...
class FrontPageEndpointsImpl(dispatchActor: ActorRef)(implicit ec: ExecutionContext, mat: Materializer) {
  ...
  val fileOptions: FilesOptions[Future] = FilesOptions.default.withUseGzippedIfAvailable
  ...
  private def getEmbeddedSnippet(snippetId: SnippetId, theme: Option[ColorScheme.Value]): Future[EmbeddedSnippet] =
    Future {
