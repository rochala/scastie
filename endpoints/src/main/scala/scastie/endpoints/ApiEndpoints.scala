...
object ApiEndpoints {
  ...
  val runEndpoint: PublicEndpoint[Inputs, Unit, SnippetId, Any] = publicEndpoint.post
    .in("run")
    .in(jsonBody[Inputs])
    .out(jsonBody[SnippetId])
    .description(
  ...
  val formatEndpoint: PublicEndpoint[FormatRequest, Unit, FormatResponse, Any] = publicEndpoint.post
    .in("format")
    .in(jsonBody[FormatRequest])
    .out(jsonBody[FormatResponse])
    .name("Format snippet")

  val saveEndpoint: Endpoint[OptionalUserSession, Inputs, String, SnippetId, Any] = baseEndpoint.post
    .in("save")
    .in(jsonBody[Inputs])
    .out(jsonBody[SnippetId])

  val forkEndpoint: Endpoint[OptionalUserSession, EditInputs, String, SnippetId, Any] = baseEndpoint.post
    .in("fork")
    .in(jsonBody[EditInputs])
    .out(jsonBody[SnippetId])
    .description(
  ...
  val deleteEndpoint: Endpoint[UserSession, SnippetId, String, Boolean, Any] = secureApiEndpoint.delete
    .in("delete")
    .in(jsonBody[SnippetId])
    .out(jsonBody[Boolean])

  val updateEndpoint: Endpoint[UserSession, EditInputs, String, SnippetId, Any] = secureApiEndpoint.post
    .in("update")
    .in(jsonBody[EditInputs])
    .out(jsonBody[SnippetId])
    .description(
  ...
  val userSettingsEndpoint: Endpoint[UserSession, Unit, String, User, Any] = secureApiEndpoint.get
    .in("user" / "settings")
    .out(jsonBody[User])

  val userSnippetsEndpoint: Endpoint[UserSession, Unit, String, List[SnippetSummary], Any] = secureApiEndpoint.get
    .in("user" / "snippets")
    .out(jsonBody[List[SnippetSummary]])

  val snippetApiEndpoint = {
    SnippetMatcher
      .getApiSnippetEndpoint(publicEndpoint.in("snippets"))
      .map(
        _.errorOut(statusCode(StatusCode.NotFound))
