...
case class ScastieBackend(scastieId: UUID, serverUrl: Option[String], scope: BackendScope[Scastie, ScastieState]) {
  ...
  val logout: Reusable[Callback] = Reusable.always(
    Callback.future(restApiClient.logout.map(_ => Callback.empty)) >>
      scope.modState(_.setUser(None))
  )
  ...
  val save: Reusable[AsyncCallback[Option[SnippetId]]] = Reusable.always(
    scope.state.asAsyncCallback.flatMap { state =>
      AsyncCallback.fromFuture(
        restApiClient.save(state.inputs).map(snippetId => Some(snippetId))
      )
    }
  )
